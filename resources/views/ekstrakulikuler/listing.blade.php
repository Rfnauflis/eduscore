                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                            <thead>
                                <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                                    <th class="min-w-125px">Nama Ekstra</th>
                                    <th class="min-w-125px">Nama Pembina</th>
                            ...
                            <tbody class="fw-semibold text-gray-900">
                                @foreach ($ekstras as $ekstra)
                                    <tr>
                                        <td>{{ $ekstra->name }}</td>
                                        <td>{{ $ekstra->pembina->name }}</td>
                                        <td class="text-end">
                                            @if (auth()->check() && auth()->guard('student')->check())
                                                <!-- Student Action: Button Daftar -->
                                                <a href="{{ route('ekstrakulikuler.register', $ekstra->id) }}" 
                                                   class="btn btn-sm btn-light-primary">Daftar</a>
